<template>
  <div class="foot-nav panel">
    ...
    <el-breadcrumb separator="/">
      ...
      <el-breadcrumb-item>用户区域</el-breadcrumb-item>
    ...
    >
      ...
      <el-table-column prop="username" label="用户名" width="180" sortable>
        <template slot-scope="scope">
          <div slot="reference" class="name-wrapper">
            <el-tag size="medium">{{ scope.row.nickname ?  scope.row.nickname : scope.row.username }}</el-tag>
          </div>
        </template>
      </el-table-column>
      ...
      <el-table-column prop="icon" label="用户头像" width="180">
        <template slot-scope="scope">
    ...
    <el-dialog :title="flagText + '商品分类'" :visible.sync="dialogFormVisible" >
      <el-form :model="form" :rules="rules" ref="ruleForm">
        ...
        <el-form-item label="真实姓名" :label-width="formLabelWidth" prop="nickname">
          <el-input v-model="form.nickname"  placeholder="可以不暴露真实姓名" autocomplete="off"></el-input>
        </el-form-item>
        ...
        <el-form-item label="头像地址" :label-width="formLabelWidth" prop="avatar">
          <el-input v-model="form.avatar" placeholder="不填就是默认" autocomplete="off"></el-input>
        </el-form-item>
...
<script>
	...
	export default {
		...
		created() {
			...
			/* 拉取数据 */
			this.$store.dispatch('handleFetch', 'Users/list.php')
		...
		methods: {
			...
			handleDelete(ids) {
				...
				let backURL = 'Users/delete.php?id=' + ids
				/* 处理删除 */
			...
			handleConfirm() {
				...
				let result = Object.assign(field, this.form)
				// console.log(result)
			...
			handleChange(id, item) {
				...
				this.$confirm('此操作将切换权限, 是否继续?', '提示', {
					...
					this.$store.dispatch('handleCheck', { id, checked: item.checked, backURL: 'Users/state.php' })
