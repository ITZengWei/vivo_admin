...
<script>
	...
	/* 后台请求的基础地址 */
  const baseURL = 'http://localhost:88/vivo_interface/Category/'
  ...
  let field =  {
    ...
    created() {
	    /* 拉取数据 */
	    this.$store.dispatch('handleFetch', 'Category/list.php')
    },
		computed: {
			/* 表格数据，store 的状态 */
    ...
    methods: {
      ...
      handleDelete(ids) {
	      ...
	      let backURL = 'Category/delete.php?id=' + ids
	      /* 处理删除 */
      ...
      handleChange(id, item) {
	      ...
	      this.$confirm('此操作将切换权限, 是否继续?', '提示', {
		      ...
		      this.$store.dispatch('handleCheck', { id, checked: item.checked, backURL: 'Category/state.php' })
