<template>
  <div class="foot-nav panel">
    ...
    <el-breadcrumb separator="/">
      ...
      <el-breadcrumb-item>底部区域</el-breadcrumb-item>
    ...
    >
      ...
      <el-table-column prop="icon" label="默认图标" width="180">
        <template slot-scope="scope">
          <i :class="scope.row.icon"></i>
        </template>
      </el-table-column>

      <el-table-column prop="iconed" label="选中图标" width="180">
        <template slot-scope="scope">
          <i :class="scope.row.iconed"></i>
        </template>
      </el-table-column>
    ...
    <el-dialog :title="flagText + '底部导航'" :visible.sync="dialogFormVisible" >
      <el-form :model="form" :rules="rules" ref="ruleForm">
        ...
        <el-form-item label="选中图标" :label-width="formLabelWidth" prop="iconed">
...
<script>
  ...
  import { mapState } from 'vuex'
	...
	/* 后台请求的基础地址 */
  const baseURL = 'http://localhost:88/vivo_interface/FootNav/'
  ...
  let field =  {
    ...
    data() {
        ...
        /* 表单校验 */
				rules: {
					...
					icon: [
						{ required: true, message: '请输入默认图标', trigger: 'blur' },
          ],
					iconed: [
						{ required: true, message: '请输入选中图标', trigger: 'blur' },
					],
					path: [
						{ required: true, message: '请输入对应路径', trigger: 'change' },
					],
					checked: [
						{ required: true, message: '请确认审核哦~', trigger: 'change' },
          ]
				},
      }
    },
    created() {
			/* 拉取数据 */
      this.$store.dispatch('handleFetch', 'FootNav/list.php')
    ...
    methods: {
      ...
      /* 删除,批量删除 表格列表 */
      handleDelete(ids) {
	      let backURL = 'FootNav/delete.php?id=' + ids
	      /* 处理删除 */
      ...
      handleChange(id, item) {
	      ...
	      let result = window.confirm('是否确认审核')
        if (result) {
          this.$store.dispatch('handleCheck', { id, checked: item.checked, backURL: 'FootNav/state.php' })
        } else {
